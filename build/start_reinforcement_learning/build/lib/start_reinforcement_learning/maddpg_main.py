...
class MADDPGNode(Node):
    def __init__(self, map_number, robot_number):
        ...
        self.weights_base_dir = os.path.join(base_path, 'start_reinforcement_learning', 'deep_learning_weights', 'maddpg')
            ...
            if not evaluate:
                ...
                if avg_score > best_score:
                    ...
                    for agent in maddpg_agents.agents:
                        ...
                        agent.target_actor.chkpt_file = os.path.join(best_model_dir, os.path.basename(agent.target_actor.chkpt_file))
                        agent.target_critic.chkpt_file = os.path.join(best_model_dir, os.path.basename(agent.target_critic.chkpt_file))
...
def main(args=None):
    ...
    try:
        ...
        print(f"Error initializing MADDPG node: {e}")
        import traceback
