...
class MultiAgentReplayBuffer:
    ...
    def store_transition(self, raw_obs, state, action, reward, 
        ...
        # Process log_probs to ensure they're numeric
        processed_log_probs = {}
        if isinstance(log_probs, dict):
            for agent_id, log_prob in log_probs.items():
                # Ensure each log_prob is a float
                if log_prob is None:
                    processed_log_probs[agent_id] = 0.0
                else:
                    try:
                        processed_log_probs[agent_id] = float(log_prob)
                    except (ValueError, TypeError):
                        processed_log_probs[agent_id] = 0.0
        else:
            # If not a dict, use as is (will be handled during sampling)
            processed_log_probs = log_probs
            
        self.current_episode['log_probs'].append(processed_log_probs)
    ...
    def sample_buffer(self):
        ...
        # Carefully convert log_probs to ensure they're all floats
        try:
            # First ensure all elements are numeric
            numeric_log_probs = []
            for log_prob in all_log_probs:
                if isinstance(log_prob, dict):
                    # Extract values from dict and convert to list of floats
                    values = [float(v) if v is not None else 0.0 for v in log_prob.values()]
                    numeric_log_probs.append(values)
                elif isinstance(log_prob, (list, np.ndarray)):
                    # Convert list/array elements to floats
                    values = [float(v) if v is not None else 0.0 for v in log_prob]
                    numeric_log_probs.append(values)
                else:
                    # Single value
                    numeric_log_probs.append(float(log_prob) if log_prob is not None else 0.0)
            
            # Convert to numpy array with proper dtype
            all_log_probs = np.array(numeric_log_probs, dtype=np.float32)
        except Exception as e:
            print(f"Error converting log_probs: {e}")
            # Create a dummy array with the right shape
            if len(all_advantages) > 0 and isinstance(all_advantages, np.ndarray):
                if len(all_advantages.shape) > 1:
                    all_log_probs = np.zeros((len(all_advantages), all_advantages.shape[1]), dtype=np.float32)
                else:
                    all_log_probs = np.zeros(len(all_advantages), dtype=np.float32)
            else:
                all_log_probs = np.zeros(len(all_states), dtype=np.float32)
